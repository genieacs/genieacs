import { logIn as apiLogIn } from "./api-client.ts";
import * as notifications from "./notifications.ts";
import * as overlay from "./overlay.ts";
import { redirect } from "./router.ts";
import { renderChangePasswordForm } from "./change-password-component.ts";
import { div, h2, form, label, input, button, span } from "./dom.ts";

// Create the GenieACS logo SVG
function createLogoSvg(): SVGSVGElement {
  const svgEl = document.createElementNS("http://www.w3.org/2000/svg", "svg");
  svgEl.setAttribute("class", "mx-auto h-14 w-auto");
...
// Create the lock icon SVG
function createLockIcon(): SVGSVGElement {
  const svgEl = document.createElementNS("http://www.w3.org/2000/svg", "svg");
  svgEl.setAttribute(
    "class",
    "h-5 w-5 text-cyan-500 group-hover:text-cyan-400",
  );
...
export async function init(args: URLSearchParams): Promise<void> {
  if (window.username) await redirect(args.get("continue") || "/");
}
...
export function createPage(): HTMLElement {
  ...
  const submitButton = button(
    {
      type: "submit",
      class:
        "group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-cyan-600 hover:bg-cyan-700 focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500",
    },
    span(
  ...
  return div(
    ...
    div(
      { class: "max-w-md w-full flex flex-col gap-8" },
      div(
        createLogoSvg(),
        h2(
          { class: "mt-6 text-center text-3xl font-extrabold text-stone-900" },
          "Log in to continue",
        ),
      ),
      form(
        { class: "mt-8 flex flex-col gap-6", onsubmit: logIn },
        div(
          { class: "rounded-md shadow-xs -space-y-px" },
          div(
            ...
            input({
              ...
              required: true,
              class:
                "appearance-none rounded-none relative block w-full px-3 py-2 border border-stone-300 placeholder-stone-500 text-stone-900 rounded-t-md focus:outline-hidden focus:ring-cyan-500 focus:border-cyan-500 focus:z-10 sm:text-sm",
          ...
          div(
            ...
            input({
              ...
              required: true,
              class:
                "appearance-none rounded-none relative block w-full px-3 py-2 border border-stone-300 placeholder-stone-500 text-stone-900 rounded-b-md focus:outline-hidden focus:ring-cyan-500 focus:border-cyan-500 focus:z-10 sm:text-sm",
        ...
        div(
          ...
          div(
            ...
            input({
              ...
              checked: remember,
              class:
                "h-4 w-4 text-cyan-700 focus:ring-cyan-500 border-stone-300 rounded-sm",
              onchange: (e) => {
                remember = (e.target as HTMLInputElement).checked;
              },
            }),
            label(
              { for: "remember", class: "ml-2 block text-sm text-stone-900" },
              "Remember me",
            ),
          ),
          div(
