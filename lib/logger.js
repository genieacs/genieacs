...
function flatten(details) {
  if (details.currentRequest) {
    details.deviceId = details.currentRequest.sessionData.deviceId;
    details.remoteAddress = details.currentRequest.httpRequest.connection.remoteAddress;
    delete details.currentRequest;
  }
  ...
  if (details.acsRequest) {
    details.acsRequestName = details.acsRequest.type;
    if (details.acsRequest.commandKey)
      details.acsRequestCommandKey = details.acsRequest.commandKey;
    delete details.acsRequest;
  }

  if (details.cpeRequest) {
    if (details.cpeRequest.type === "Inform") {
      details.informEvent = details.cpeRequest.event.join(",");
      details.informRetryCount = details.cpeRequest.retryCount;
    }
    else {
      details.cpeRequestName = details.cpeRequest.type;
      if (details.cpeRequest.commandKey)
        details.cpeRequestCommandKey = details.cpeRequest.commandKey;
    }
    delete details.cpeRequest;
  }

  if (details.cpeFault) {
    details.cpeFaultCode = details.cpeFault.faultCode;
    details.cpeFaultString = details.cpeFault.faultString;
    delete details.cpeFault;
  }
