...
class VersionedMap {
  ...
  get(key, rev) {
    if (rev == null) rev = this._revision;
    ...
    for (let i = Math.min(v.length - 1, rev); i >= 0; --i) {
      if (i in v) {
        if (v[i] === NONEXISTENT) return UNDEFINED;
        else return v[i];
      }
    }
  ...
  has(key, rev) {
    ...
    for (let i = Math.min(v.length - 1, rev); i >= 0; --i) {
      if (i in v) {
        if (v[i] === NONEXISTENT) return false;
        else return true;
      }
    }
  ...
  set(key, value, rev) {
    ...
    if (rev < v.length - 1) {
      if (compareEquality(value, old)) return;
      else throw new Error("Cannot modify old revisions");
    }
  ...
  delete(key, rev) {
    ...
    if (rev < v.length - 1) throw new Error("Cannot modify old revisions");
  ...
  getRevisions(key) {
    ...
    for (const i in v) {
      if (v[i] === NONEXISTENT) res.delete |= 1 << i;
      else res[i] = v[i];
    }
  ...
  setRevisions(key, revisions) {
    ...
    for (let i = 0; i < minKey; ++i) --this._sizeDiff[i];

    this.dirty |= del;

    for (let i = 0; del > 0; del >>= 1, ++i) if (del & 1) rev[i] = NONEXISTENT;
  ...
  getDiff(key) {
    ...
    if (current === NONEXISTENT) {
      if (0 in revisions) return [key, revisions[0]];
      else return [key];
    } else if (0 in revisions) {
      return [key, revisions[0], current];
    } else {
      // TODO don't use sparse arrays?
  ...
  *diff() {
    for (const pair of this.map) {
      const current = pair[1][pair[1].length - 1];
      if (current === NONEXISTENT) {
        if (0 in pair[1]) yield [pair[0], pair[1][0]];
        else yield [pair[0]];
      } else if (0 in pair[1]) {
  ...
  collapse(revision) {
    ...
    for (const pair of this.map) {
      ...
      if (last === NONEXISTENT) {
        ...
        if (v.some(() => true)) v[revision] = NONEXISTENT;
