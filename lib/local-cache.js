...
function computeHash() {
  ...
  for (let k of keys) h.update(provisions[k].md5);

  keys = Object.keys(virtualParameters).sort();
  h.update(JSON.stringify(keys));
  for (let k of keys) h.update(virtualParameters[k].md5);
      ...
      db.getPresets((err, res) => {
        ...
        db.getObjects((err, objects) => {
          ...
          for (let preset of res) {
            ...
            let precondition = query.convertMongoQueryToFilters(
...
function getPresets(callback) {
  if (Date.now() < nextRefresh) return callback(null, hash, presets);
...
function getFiles(callback) {
  if (Date.now() < nextRefresh) return callback(null, hash, files);
