...
function writeResponse(sessionContext, res, close) {
  ...
  if (compress) {
    compress(res.data, (err, data) => {
      if (err) return throwError(err, sessionContext.httpResponse);
...
function applyPresets(sessionContext) {
  ...
  localCache.getPresets((err, presetsHash, presets) => {
    ...
    if (sessionContext.faults) {
      for (let [channel, fault] of Object.entries(sessionContext.faults)) {
    ...
    for (const preset of presets) {
      ...
      for (let [k, v] of Object.entries(preset.events)) {
      ...
      for (let k of Object.keys(preset.precondition)) {
    ...
    const declarations = [];
    for (let v of Object.values(parameters))
    ...
    session.rpcRequest(
      ...
      (err, flt, reqId, acsReq) => {
        ...
        for (let [channel, provisions] of Object.entries(
...
function nextRpc(sessionContext) {
  session.rpcRequest(sessionContext, null, (err, fault, id, acsRequest) => {
    ...
    for (let [channel, flags] of Object.entries(sessionContext.channels)) {
...
function endSession(sessionContext, callback) {
  ...
  db.saveDevice(
    ...
    err => {
      ...
      if ((counter -= 2) === 1) return callback(null, sessionContext.new);
    }
  );
...
function getSession(connection, sessionId, callback) {
  ...
  setTimeout(() => {
    cache.pop(`session_${sessionId}`, (err, sessionContextString) => {
      if (err || !sessionContextString) return callback(err);
...
function onConnection(socket) {
  ...
  socket.on("close", () => {
    ...
    if (timeout <= 0) return logger.accessError(timeoutMsg);
...
function processRequest(sessionContext, rpc) {
  if (rpc.cpeRequest) {
    ...
    session.timeoutOperations(sessionContext, (err, faults, operations) => {
      ...
      for (let i of faults) {
        for (let [k, v] of Object.entries(operations[i].retries))
...
function listener(httpRequest, httpResponse) {
  ...
  stream.on("end", () => {
    ...
    getSession(httpRequest.connection, sessionId, (err, sessionContext) => {
      if (err) return throwError(err, httpResponse);
        ...
        (err, _sessionContext) => {
          ...
          getDueTasksAndFaultsAndOperations(
            ...
            (err, dueTasks, faults, operations, cacheUntil) => {
              ...
              for (let [k, v] of Object.entries(_sessionContext.faults)) {
