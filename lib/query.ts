...
function matchType(src, dst): string | number | boolean {
...
export function testFilter(obj, filter): boolean {
...
export function convertMongoQueryToFilters(query, filters = {}): {} {
  for (let [k, v] of Object.entries(query)) {
    if (k[0] === "$") {
      if (k === "$and")
        for (const vv of v as any[]) convertMongoQueryToFilters(vv, filters);
      ...
      if (common.typeOf(v) === common.OBJECT_TYPE) {
        ...
        v = (v as string).replace(/[^a-zA-Z0-9-]+/g, "_");
        filters[`Tags.${v}`] = true;
      }
    } else {
      switch (k) {
