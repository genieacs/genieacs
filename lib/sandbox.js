...
class SandboxDate {
  ...
  static now() {
    return state.sessionContext.timestamp;
  }
};

function random() {
  if (!state.rng)
    state.rng = seedrandom(sessionContext.timestamp);
    ...
    for (let attrName of attributes) {
      Object.defineProperty(this, attrName, {get: function() {
        ...
        if (state.revision !== unpackedRevision) {
          ...
          unpacked = device.unpack(state.sessionContext.deviceData, path, state.revision + 1);
        }

        if (!unpacked.length)
          return undefined;

        let attr = state.sessionContext.deviceData.attributes.get(unpacked[0], state.revision + 1)[attrName];
...
function errorToFault(err) {
  ...
  let stackTrimIndex = err.stack.match(/\s+at\s[^\s]+\s+at\s[^\s]+\s\(vm\.js.+\)/).index;
  ...
  return {
    ...
    detail: {
      name: err.name,
      message: err.message,
      stack: err.stack.slice(0, stackTrimIndex)
    }
  }
}


function run(script, globals, sessionContext, startRevision, maxRevision, callback) {
  state = {
  ...
  for (let key of Object.keys(_state.extensions)) {
    counter += 2;
    extensions.run(_state.extensions[key], function(err, fault, res) {
      if (err || fault) {
        if (counter & 1)
          callback(err, fault);
        return counter = 0;
      }
      _state.sessionContext.extensionsCache[key] = res;
      if ((counter -= 2) === 1)
        if (status === 2)
