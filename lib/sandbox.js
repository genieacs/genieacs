...
function runExtension(sessionContext, key, extCall, callback) {
  ...
  if (re[key]) {
    ...
    extensions.run(extCall, (err, fault, res) => {
      ...
      for (let c of callbacks) c(err, fault);
    });
  }
}
    ...
    for (let attrName of attributes) {
      Object.defineProperty(this, attrName, {
        get: function() {
          ...
          let attr = state.sessionContext.deviceData.attributes.get(
            unpacked[0],
            state.revision
          )[attrName];
...
function errorToFault(err) {
  ...
  if (err.stack) {
    ...
    let stackTrimIndex = fault.detail.stack.match(
...
function run(
  ...
  for (let n of Object.keys(context)) delete context[n];
  ...
  try {
    ...
    else return callback(null, errorToFault(err));
  }
