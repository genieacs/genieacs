  ...
  db.devicesCollection.findOne({ _id: deviceId }, proj, (err, device) => {
    ...
    if (config.auth && config.auth.connectionRequest) {
      ...
      if (config.auth.connectionRequest.length > 4) {
        return config.auth.connectionRequest(
          deviceId,
          connectionRequestUrl,
          username,
          password,
...
function watchTask(deviceId, taskId, timeout, callback) {
  setTimeout(() => {
    db.tasksCollection.findOne({ _id: taskId }, { _id: 1 }, (err, task) => {
      ...
      if (!task) return callback(null, "completed");

      const q = { _id: `${deviceId}:task_${taskId}` };
      db.faultsCollection.findOne(q, { _id: 1 }, (err, fault) => {
        if (err) return callback(err);

        if (fault) return callback(null, "fault");

        if ((timeout -= 500) <= 0) return callback(null, "timeout");
...
function insertTasks(tasks, callback) {
  ...
  else if (!tasks || tasks.length === 0) return callback(null, tasks || []);
  ...
  for (let task of tasks) {
    sanitizeTask(task, t => {
      ...
      if (--counter === 0) db.tasksCollection.insert(tasks, callback);
    });
  }
}
