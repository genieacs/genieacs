...
function udpConReq(address, un, key, callback) {
  ...
  const sig = crypto.createHmac("sha1", key).update(`${ts}${id}${un}${cn}`).digest("hex");
  const uri = `http://${address}?ts=${ts}&id=${id}&un=${un}&cn=${cn}&sig=${sig}`;
  const message = Buffer.from(`GET ${uri} HTTP/1.1\r\nHost: ${address}\r\n\r\n`);

  const client = dgram.createSocket({type: "udp4", reuseAddr: true});
  const UDP_CONNECTION_REQUEST_PORT = config.get("UDP_CONNECTION_REQUEST_PORT");
...
function httpConReq(url, username, password, allowBasicAuth, timeout, callback) {
  ...
  const request = http.get(options, (res) => {
    if (res.statusCode === 401 && res.headers["www-authenticate"]) {
      const authHeader = auth.parseAuthHeader(res.headers["www-authenticate"]);
      if (authHeader.method === "Basic") {
        if (!allowBasicAuth) {
          request.abort();
          return callback(new Error("Basic HTTP authentication not allowed"));
        }
        options.headers = {
...
function connectionRequest(deviceId, callback) {
  const CONNECTION_REQUEST_TIMEOUT = config.get("CONNECTION_REQUEST_TIMEOUT", deviceId);
  const CONNECTION_REQUEST_ALLOW_BASIC_AUTH = config.get("CONNECTION_REQUEST_ALLOW_BASIC_AUTH", deviceId);
  const proj = {
  ...
  db.devicesCollection.findOne({_id: deviceId}, proj, (err, device) => {
    ...
    let managementServer, connectionRequestUrl, udpConnectionRequestAddress, username, password;
    ...
    if (managementServer.UDPConnectionRequestAddress)
      udpConnectionRequestAddress = managementServer.UDPConnectionRequestAddress._value;
    ...
    const conReq = () => {
      const udpSent = udpConReq(udpConnectionRequestAddress, username, password, (err) => {
        if (err)
          throw err;
      });
...
function watchTask(deviceId, taskId, timeout, callback) {
  setTimeout(() => {
    db.tasksCollection.findOne({_id: taskId}, {_id: 1}, (err, task) => {
      ...
      db.faultsCollection.findOne(q, {_id: 1}, (err, fault) => {
        if (err)
          return callback(err);
...
function sanitizeTask(task, callback) {
...
function insertTasks(tasks, callback) {
  if (tasks && common.typeOf(tasks) !== common.ARRAY_TYPE)
    tasks = [tasks];
  ...
  for (let task of tasks)
    sanitizeTask(task, (t) => {
      if (t.uniqueKey)
        db.tasksCollection.remove({device: t.device, uniqueKey: t.uniqueKey}, () => {});

      if (--counter === 0)
        db.tasksCollection.insert(tasks, callback);
    });
}
  ...
  db.tasksCollection.remove({"device": deviceId}, (err) => {
    ...
    db.devicesCollection.remove({"_id": deviceId}, (err) => {
      ...
      db.faultsCollection.remove({
        ...
        if (err)
          return callback(err);
        db.operationsCollection.remove({
          "_id": {
            "$regex": `^${common.escapeRegExp(deviceId)}\\:`
          }
        }, (err) => {
