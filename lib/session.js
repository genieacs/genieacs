...
export function init(deviceId, cwmpVersion, timeout, callback) {
...
export function inform(sessionContext, rpcReq, callback) {
...
export function transferComplete(sessionContext, rpcReq, callback) {
  const revision =
...
export function timeoutOperations(sessionContext, callback) {
  const revision =
    (sessionContext.revisions[sessionContext.revisions.length - 1] || 0) + 1;
  sessionContext.deviceData.timestamps.revision = revision;
  sessionContext.deviceData.attributes.revision = revision;
...
export function addProvisions(sessionContext, channel, provisions) {
...
export function clearProvisions(sessionContext) {
  // Multiply by two because every iteration is two
  // phases: read and update
  const MAX_ITERATIONS =
    localCache.getConfig(
...
export function rpcRequest(sessionContext, _declarations, callback) {
...
export function rpcResponse(sessionContext, id, rpcRes, callback) {
...
export function rpcFault(sessionContext, id, faultResponse, callback) {
  const rpcReq = sessionContext.rpcRequest;
  delete sessionContext.syncState;
  delete sessionContext.rpcRequest;
...
export function deserialize(sessionContextString, callback) {
...
export function serialize(sessionContext, callback) {
  const deviceData = [];
