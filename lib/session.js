...
const provisionsCache = new WeakMap();
const virtualParametersCache = new WeakMap();
...
function init(deviceId, cwmpVersion, timeout, callback) {
  ...
  localCache.getProvisionsAndVirtualParameters(
    (err, hash, provisions, virtualParameters) => {
...
function loadParameters(sessionContext, callback) {
  if (!sessionContext.toLoad || !sessionContext.toLoad.size) return callback();
...
function transferComplete(sessionContext, rpcReq, callback) {
  ...
  if (!operation) return callback(null, { name: "TransferCompleteResponse" });
  ...
  if (rpcReq.faultStruct && rpcReq.faultStruct.faultCode !== "0") {
    return revertDownloadParameters(
      sessionContext,
...
function revertDownloadParameters(sessionContext, instance, callback) {
  ...
  loadParameters(sessionContext, err => {
    ...
    let toClear = device.set(sessionContext.deviceData, p, timestamp, {
      value: [
        timestamp,
        [
...
function runVirtualParameters(
  ...
  let allClear = [];
  let counter = 3;
  const allVirtualParameters = virtualParametersCache.get(sessionContext);
...
function runDeclarations(sessionContext, declarations) {
  ...
  function mergeAttributeValues(p, attrs) {
  ...
  for (const declaration of declarations) {
    let path = common.addPathMeta(declaration[0]);
    let unpacked;
    ...
    if (path.alias) {
      const aliasDecs = device.getAliasDeclarations(path, declaration[1] || 1);
      for (const ad of aliasDecs) {
        const p = sessionContext.deviceData.paths.add(ad[0]);
        allDeclareTimestamps.set(
          p,
          Math.max(ad[1] || 1, allDeclareTimestamps.get(p) || 0)
        );
        let attrTrackers;
        if (ad[2]) {
      ...
      for (const u of unpacked) {
        allDeclareTimestamps.set(
          u,
          Math.max(declaration[1] || 1, allDeclareTimestamps.get(u) || 0)
        );
        if (declaration[2]) mergeAttributeTimestamps(u, declaration[2]);
      }
    } else {
      path = sessionContext.deviceData.paths.add(path);
      allDeclareTimestamps.set(
        path,
        Math.max(declaration[1] || 1, allDeclareTimestamps.get(path) || 0)
      );
      if (declaration[2]) mergeAttributeTimestamps(path, declaration[2]);
    ...
    if (declaration[4]) {
      if (path.alias | path.wildcard) {
        ...
        for (const u of unpacked) mergeAttributeValues(u, declaration[4]);
      } else {
    ...
    if (declaration[3] != null) {
      ...
      if (Array.isArray(path[path.length - 1])) {
        keys = {};
        for (let i = 0; i < path[path.length - 1]; i += 2) {
...
function rpcRequest(sessionContext, _declarations, callback) {
    ...
    return run(
      ...
      (err, fault, ret, decs, toClear) => {
        ...
        for (const d of decs) {
          ...
          if (d[1] > sessionContext.timestamp) d[1] = sessionContext.timestamp;
          for (const [k, v] of Object.entries(d[2])) {
            if (v > sessionContext.timestamp)
              d[2][k] = sessionContext.timestamp;
          }

          for (const ad of device.getAliasDeclarations(d[0], 1))
  ...
  if (!sessionContext.syncState) return callback();
  ...
  if (!ret) return rpcRequest(sessionContext, null, callback);
...
function generateSetRpcRequest(sessionContext) {
  ...
  for (const [p, t] of syncState.downloadsDownload) {
    const attrs = deviceData.attrs.get(p);
...
function processDeclarations(
  ...
  const paths = sessionContext.deviceData.paths.find([], false, true, 99);
  paths.sort((a, b) => {
    if (a.wildcard === b.wildcard) return a.length - b.length;
  ...
  function func(leafParam, leafIsObject, leafTimestamp, _paths) {
    ...
    for (const child of Object.keys(children)) {
      ...
      if (
        ((currentPath.wildcard ^ children[child][0].wildcard) &
...
function clear(sessionContext, toClear, callback) {
  if (!toClear || !toClear.length) return callback();
...
function deserialize(sessionContextString, callback) {
  localCache.getProvisionsAndVirtualParameters(
    (err, hash, provisions, virtualParameters) => {
      ...
      if (sessionContext.presetsHash && sessionContext.presetsHash !== hash)
      ...
      for (const decs of sessionContext.declarations)
        for (const d of decs) common.addPathMeta(d[0]);
