...
export function parseAlias(pattern, start, res): number {
  const aliases = [];
  let i = start;
...
export function parsePath(pattern, start, res): number;
export function parsePath(pattern, start?): ParamPath;
export function parsePath(pattern, start?, res?): number | ParamPath {
  const path = [] as ParamPath;
  path.wildcard = 0;
  path.alias = 0;
  let i = start || 0;
...
export function addPathMeta(path): ParamPath {
