...
export function postTasks(deviceId, tasks) {
  return new Promise((resolve, reject) => {
    let connectionRequestStatus;
    const promises = [];
...
export function updateTags(deviceId, tags) {
  return Promise.all(
...
export function putResource(resource, id, data) {
  return new Promise((resolve, reject) => {
    if (resource === "config") {
      db.putConfig(id, data)
        .then(() => {
          cache.del("presets_hash", err => {
            if (err) return void reject(err);
            resolve();
...
export function putFile(id, metadata, fileStream) {
  return new Promise((resolve, reject) => {
    const options = url.parse(
