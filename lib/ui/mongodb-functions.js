...
export function processDeviceFilter(filter) {
...
export function processTasksFilter(filter) {
  return map(filter, exp => {
    if (!Array.isArray(exp)) return exp;
    if (["=", "<>", ">", ">=", "<", "<="].includes(exp[0])) {
      const e = exp.slice();
...
export function processFaultsFilter(filter) {
  return map(filter, exp => {
    if (!Array.isArray(exp)) return exp;
    if (["=", "<>", ">", ">=", "<", "<="].includes(exp[0])) {
      const e = exp.slice();
...
export function filterToMongoQuery(filter, negate = false, res = {}) {
  const op = filter[0];
...
export function flattenDevice(device) {
...
export function flattenFault(fault) {
...
export function flattenTask(task) {
...
export function preProcessProvision(data) {
  return data.script;
}

export function preProcessVirtualParameters(data) {
  return data.script;
}
