...
export function listener(request, response) {
  const chunks = [];
  let bytes = 0;
  ...
  request.addListener("end", () => {
    ...
    if (PRESETS_REGEX.test(urlParts.pathname)) {
                ...
                apiFunctions.connectionRequest(deviceId, err => {
                  ...
                  const taskTimeout =
                    (urlParts.query.timeout &&
                      parseInt(urlParts.query.timeout)) ||
      ...
      if (urlParts.query.limit) cur.limit(parseInt(urlParts.query.limit));
