...
function listener(request, response) {
  ...
  request.addListener("end", () => {
    ...
    if (PRESETS_REGEX.test(urlParts.pathname)) {
      const presetName = querystring.unescape(PRESETS_REGEX.exec(urlParts.pathname)[1]);
      ...
      const objectName = querystring.unescape(OBJECTS_REGEX.exec(urlParts.pathname)[1]);
      if (request.method === "PUT") {
      ...
      const provisionName = querystring.unescape(PROVISIONS_REGEX.exec(urlParts.pathname)[1]);
      if (request.method === "PUT") {
        const object = {
          _id: provisionName,
      ...
      const virtualParameterName = querystring.unescape(VIRTUAL_PARAMETERS_REGEX.exec(urlParts.pathname)[1]);
      if (request.method === "PUT") {
        const object = {
      ...
      if (request.method === "DELETE") {
        const faultId = querystring.unescape(FAULTS_REGEX.exec(urlParts.pathname)[1]);
        ...
        db.faultsCollection.remove({_id: faultId}, (err) => {
          ...
          if (channel.startsWith("task_")) {
            ...
            return db.tasksCollection.remove({_id: objId}, (err) => {
              if (err)
                return throwError(err, response);
      ...
      if (request.method === "POST") {
        const deviceId = querystring.unescape(DEVICE_TASKS_REGEX.exec(urlParts.pathname)[1]);
        if (body.length) {
            ...
            cache.del(`${deviceId}_tasks_faults_operations`, (err) => {
              ...
              if (urlParts.query.connection_request != null) {
                apiFunctions.connectionRequest(deviceId, (err) => {
                  ...
                  const DEVICE_ONLINE_THRESHOLD = config.get("DEVICE_ONLINE_THRESHOLD", deviceId);
                  apiFunctions.watchTask(deviceId, task._id, DEVICE_ONLINE_THRESHOLD, (err, status) => {
                    if (err)
                      return throwError(err, response);
      ...
      if (!action || action === "/") {
        if (request.method === "DELETE") {
          db.tasksCollection.findOne(
            ...
            db.tasksCollection.remove(
                {"_id": new mongodb.ObjectID(taskId)}, (err) => {
              if (err)
                return throwError(err, response);

              db.faultsCollection.remove({_id: `${deviceId}:task_${taskId}`}, (err) => {
                if (err)
                  return throwError(err, response);
        ...
        if (request.method === "POST") {
          db.tasksCollection.findOne({"_id": new mongodb.ObjectID(taskId)}, {"device": 1}, (err, task) => {
            if (err)
              return throwError(err, response);
      ...
      const filename = querystring.unescape(FILES_REGEX.exec(urlParts.pathname)[1]);
      if (request.method === "PUT") {
        const metadata = {
      ...
      const deviceId = querystring.unescape(DELETE_DEVICE_REGEX.exec(urlParts.pathname)[1]);
      ...
      while (i++ >= 0) {
        const up = i < collectionName.length ? collectionName[i].toUpperCase() : "";
      ...
      if (urlParts.query.projection) {
        projection = {};
        for (let p of urlParts.query.projection.split(","))
          projection[p.trim()] = 1;
      }
