...
function expandValue(value) {
  if (common.typeOf(value) === common.ARRAY_TYPE) {
    let a = [];
    for (let j of value) a = a.conact(expandValue(j));
    return [a];
    ...
    if (k[0] === "$") {
      ...
      if (conditions.length > 1) {
        ...
        if (v && (v["$ne"] != null || v["$not"] != null)) {
          ...
          for (let c of conditions) newQuery["$an"].push(c);
        } else {
...
function convertMongoQueryToFilters(query, filters) {
  ...
  for (let [k, v] of Object.entries(query)) {
    if (k[0] === "$") {
      if (k === "$and")
        for (let vv of v) convertMongoQueryToFilters(vv, filters);
...
function sanitizeQueryTypes(query, types) {
  for (let [k, v] of Object.entries(query)) {
    if (k[0] === "$") {
      // Logical operator
      for (let vv of v) sanitizeQueryTypes(vv, types);
    } else if (k in types) {
      if (common.typeOf(v) === common.OBJECT_TYPE) {
        for (let [kk, vv] of Object.entries(v)) {
          switch (kk) {
