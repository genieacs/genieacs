...
function getAliasDeclarations(path, timestamp) {
  ...
  function recursive(p, prefix) {
    ...
    if (prefix.length) decs.push([pattern, timestamp, { value: timestamp }]);
    else decs.push([pattern, timestamp]);
  }
...
function clear(deviceData, path, timestamp, attributes, changeFlags = 0) {
  ...
  for (let p of deviceData.paths.find(
    path,
    true,
    true,
    ...
    for (let k in tracker) changeTrackers[k] |= tracker[k];
...
function track(deviceData, path, marker, attributes) {
  ...
  if (attributes) for (let attrName of attributes) f |= CHANGE_FLAGS[attrName];
...
function clearTrackers(deviceData, tracker) {
  if (Array.isArray(tracker)) {
    for (let v of deviceData.trackers.values())
      for (let t of tracker) delete v[t];
    for (let t of tracker) deviceData.changes.delete(t);
  } else {
    for (let v of deviceData.trackers.values()) delete v[tracker];
