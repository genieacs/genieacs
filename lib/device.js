...
function clear(deviceData, path, timestamp, attributes) {
  for (let p of deviceData.paths.all(path, timestamp ? 99 : 0)) {
    ...
    if (changeFlags)
      for (let sup of deviceData.paths.superset(p)) {
  ...
  if (attributes) {
    ...
    if (currentAttributes) {
      ...
      if (Object.keys(attributes).length) {
        if (attributes.object && attributes.object[1] === 0) {
          toClear = toClear || [];
...
function track(deviceData, path, marker, timestamp, attributeTimestamps, exist, attributeValues) {
  ...
  if (attributeTimestamps)
    for (let attrName in attributeTimestamps)
      f |= CHANGE_FLAGS[attrName];

  if (attributeValues)
    for (let attrName in attributeValues)
