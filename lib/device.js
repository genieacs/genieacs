...
export function sanitizeParameterValue(parameterValue) {
...
export function getAliasDeclarations(path, timestamp) {
  const decs = [];
    ...
    for (let i = path.length - 1; i >= 0; --i) {
      if (Array.isArray(path[i])) {
        for (let j = 0; j < path[i].length; j += 2) {
          const p = wildcardPath.slice(0, i + 1).concat(path[i][j]);
          p.alias =
...
export function set(deviceData, path, timestamp, attributes, toClear) {
...
export function track(deviceData, path, marker, attributes) {
  path = deviceData.paths.add(path);
  let f = 1;
...
export function clearTrackers(deviceData, tracker) {
