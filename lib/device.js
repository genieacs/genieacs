...
function clear(deviceData, path, timestamps, toClear) {
  if (!toClear)
    toClear = new Map();
    ...
    if (deviceData.values.exist.has(sub)) {
      ...
      for (let attrName in timestamps) {
        ...
        if (timestamps[attrName] > deviceData.timestamps[attrName].get(sub)) {
          ...
          deviceData.timestamps[attrName].set(sub, timestamps[attrName]);
        }
      }
    }
    else {
...
function clearSupersets(deviceData, path, timestamps) {
  let depth = 0;
  ...
  for (let sup of deviceData.paths.superset(path, 99)) {
    ...
    for (let attrName in deviceData.timestamps) {
      let t = deviceData.timestamps[attrName].get(sup);
      if (t <= timestamps[attrName] || t <= timestamps.exist) {
...
function set(deviceData, path, timestamps, values, toClear) {
  ...
  if (values.exist) {
    ...
    if (timestamps.exist && path.length > 1)
      toClear = set(deviceData, path.slice(0, path.length - 1),
    ...
    for (let attrName in timestamps) {
      if (deviceData.values[attrName].has(path) != (attrName in values)) {
        let inv = toClear.get(path);
        if (inv == null)
          toClear.set(path, inv = {});
        inv[attrName] = timestamps[attrName];
  ...
  return clear(deviceData, path, timestamps, toClear);
}
...
exports.set = set;
exports.clear = clear;
exports.clearSupersets = clearSupersets
