...
function connect(callback) {
  mongodb.MongoClient.connect(
    ...
    (err, client) => {
      ...
      exports.tasksCollection = tasksCollection = db.collection("tasks");
      tasksCollection.createIndex({ device: 1, timestamp: 1 });

      exports.devicesCollection = devicesCollection = db.collection("devices");
      exports.presetsCollection = presetsCollection = db.collection("presets");
      exports.objectsCollection = objectsCollection = db.collection("objects");
      exports.filesCollection = filesCollection = db.collection("fs.files");
      exports.provisionsCollection = provisionsCollection = db.collection(
        "provisions"
      );
      exports.virtualParametersCollection = virtualParametersCollection = db.collection(
        "virtualParameters"
      );
      exports.faultsCollection = faultsCollection = db.collection("faults");
      exports.operationsCollection = operationsCollection = db.collection(
        "operations"
      );
...
function disconnect() {
  if (exports.client) exports.client.close();
}
...
function getConfig(callback) {
  configCollection.find().toArray((err, res) => {
    ...
    callback(
      ...
      res.map(c => ({
        id: c["_id"],
        value: expressionParser.parse(c["value"])
      }))
    );
  });
}
