...
function setConfig(name, value, commandLineArgument) {
  ...
  function cast(val, type) {
    switch (type) {
      case "int":
        return Number(val);
      case "bool":
        return ["true", "on", "yes", "1"].includes(String(val).trim().toLowerCase());
      case "string":
        return String(val);
      case "path":
        if (!val)
  ...
  let _value = null;
  for (let [optionName, optionDetails] of Object.entries(options)) {
    let n = optionName;
...
// Configuration file
const configFile = require(path.resolve(allConfig.CONFIG_DIR, "config"));
