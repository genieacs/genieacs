...
function parameterInfoList(xml) {
  return xml.find("ParameterInfoStruct").map(e => {
    const param = e.get("Name").text();
    const value = e.get("Writable").text();
...
function parameterValueList(xml) {
  return xml.find("ParameterValueStruct").map(e => {
...
function faultStruct(xml) {
  ...
  for (const e of xml.find("SetParameterValuesFault")) {
    if (!f.setParameterValuesFault) f.setParameterValuesFault = [];
...
function request(data, cwmpVersion, warn) {
  ...
  if (rpc.cwmpVersion) {
    methodElement = xml.get(
      "/soap-env:Envelope/soap-env:Body/cwmp:*",
      NAMESPACES[rpc.cwmpVersion]
    );
  } else {
    ...
    switch (methodElement.namespace().href()) {
      ...
      case "urn:dslforum-org:cwmp-1-2":
        timeoutElement = xml.get(
          "/soap-env:Envelope/soap-env:Header/cwmp:sessionTimeout",
          NAMESPACES["1.2"]
        );
        if (timeoutElement) {
    ...
    if (!rpc.sessionTimeout) {
      ...
      if (timeoutElement) rpc.sessionTimeout = parseInt(timeoutElement.text());
    }
  }

  const idElement = xml.get(
    "/soap-env:Envelope/soap-env:Header/cwmp:ID",
    NAMESPACES[rpc.cwmpVersion]
  );
  if (idElement) rpc.id = idElement.text();

  if (
    methodElement &&
    !(XML_IGNORE_NAMESPACE && methodElement.name() === "Fault")
  ) {
