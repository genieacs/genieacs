...
function parameterInfoList(xml) {
  return xml.find("ParameterInfoStruct").map(
    e => [e.get("Name").text(), parseBool(e.get("Writable").text())]
  );
}
...
function parameterValueList(xml) {
  return xml.find("ParameterValueStruct").map(function(e) {
    ...
    if (valueType === "xsd:boolean")
     ...
     return [e.get("Name").text(), value, valueType];
  });
}
...
function SetParameterValues(xml, methodRequest) {
  ...
  for (const p of methodRequest.parameterList) {
    ...
    pvs.node("Value").attr({"xsi:type": p[2]}).text(p[1]);
  }
...
function request(httpRequest, cwmpVersion) {
  const rpc = {
    cwmpVersion: cwmpVersion
  };

  const data = httpRequest.getBody();
  ...
  rpc.id = xml.get("/soap-env:Envelope/soap-env:Header/cwmp:ID", NAMESPACES[rpc.cwmpVersion]).text();
